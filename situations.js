...
document.getElementById("buttonTryBathroom").addEventListener("click", function tryBathroomResult() {
    var frEnding = "You wake up in the middle of the street. You scramble to your feet and go read a sign that says 'France, 1792'. You dont realize the significance of that date until you hear a loud thump in the background. You turn around to watch as a bloody blade is being raised. That is a guillotine, and YOU are stuck during the bloodiest time of the French Revolution."
    var shipEnding = "You wake up in the middle of a ship. As you observe your surroundings, you notice this is an old wooden ship. You find yourself surrounded by pirates. They accept you as one of their own. Your new name is Time Beard, and you're starting your next adventure to get back to your proper time."
    var correctEnding = "You wake up back in your room. You get up and look around just to confirm that you are indeed back in your time period. You deduce that you're back in your time period because your room still looks like a mess after the party that transported you in the first place. You did it!"

    var endings = [frEnding, shipEnding, correctEnding];
    var endingIdx = Math.floor(Math.random()*endings.length);
    var ending = endings[endingIdx];
    var house = document.getElementById('house')
    var flag = document.getElementById('flag')
    var ship = document.getElementById('ship')
    var bckgrndImg = document.getElementById('scroll')
    if (ending == shipEnding) {
        document.getElementById('situationText').style.color = "white"
        document.getElementById("playAgainButton").style.color = "white"
    
    }
    if (ending == frEnding) {
        bckgrndImg.style.display = 'none'
        flag.style.display = 'inline'
        document.getElementById('situationText').style.color = "gold"
        document.getElementById("playAgainButton").style.color = "gold"
    }
    if (ending == shipEnding) {
        bckgrndImg.style.display = 'none'
    ...
    document.getElementById("situationText").innerHTML = `You sprint into the nearest restroom. You quickly check your surroundings and see a hole in the ground. You hesitate because that's disgusting. However you find it a necesary evil just in case it helps you find a way home. You peak over the edge to find... a portal? You assume that this is your shot, your only way out of this time period. You fit yourself into this very small hole. ${ending}`;
...
document.getElementById("buttonTryMartialArts").addEventListener("click", function tryMartialArtsResult() {
    ...
    document.getElementById("buttonUseSword").style.display = "none"
    document.getElementById("buttonCharge").style.display = "none"
    var maFail = "and land flat on your face. The gaurd stabs you while you're face down on the floor. YOU LOSE."
    var maSuccess = "and you actually manage to hit the guard with this fiery combo. He falls and you're victorious. You notice a few escape routes such as a barred window, a vent on the floor, and a staircase made of stone. What is your next move?"
    var maArray = [maFail, maFail, maFail, maFail, maFail, maFail, maFail, maFail, maSuccess, maSuccess]
    var maIdx = Math.floor(Math.random() * maArray.length);
    var ma = maArray[maIdx];
    document.getElementById("situationText").innerHTML = `You attempt to hit the guard with a reverse dragon kick combo. You leap high into the air, ${ma}`
    if (ma == maSuccess) {
        document.getElementById("buttonTryStairs").style.display = "inline"
        document.getElementById("buttonTryVent").style.display = "inline"
        document.getElementById("buttonTryWindow").style.display = "inline"
...
document.getElementById("playAgainButton").addEventListener("click", function againResult() {
    ...
    document.getElementById('situationText').style.color = "black"
});
